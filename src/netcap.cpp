...
void NetCap::setInterfaceName(std::string ifn) { ifName = ifn; }

std::string NetCap::getInterfaceName() const { return ifName; }
...
void NetCap::startCapture(const std::atomic<bool> &running) {
  while (running) {
    int result = pcap_dispatch(
        pcapHandle, -1,
        [](u_char *userData, const pcap_pkthdr *pkthdr, const u_char *packet) {
          ...
          ip *ipHeader = (ip *)(packet + sizeof(ether_header));
          ...
          if (ipHeader->ip_p == IPPROTO_TCP) {
            ...
            tcphdr *tcpHeader = (tcphdr *)((u_char *)ipHeader + ipHeaderLen);
            ...
            udphdr *udpHeader = (udphdr *)((u_char *)ipHeader + ipHeaderLen);
